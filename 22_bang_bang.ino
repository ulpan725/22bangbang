...
void loop()
{
  ...
  if (event_dist) {
    ...
    float filtered = ir_sensor_filtered(10, 0.5, 0); // Replace n with your desired value
    float dist = ((6762.0 / (filtered - 9.0)) - 4.0) * 10.0 - 60.0;
    Serial.print("FLT:"); Serial.print(filtered);
    float result = volt_to_distance(filtered);
    int accurate;
    if (result <= 114){
      accurate = small_distance_to_accurate(result);
    }
    if (result > 114){
      accurate = big_distance_to_accurate(result);
    }
    dist_filtered = accurate;
